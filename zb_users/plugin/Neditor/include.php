...
function ActivePlugin_Neditor()
{
    ...
    if (!$zbp->Config('Neditor')->HasKey('plugin')) { // v2.24之前版本
        ...
        if (json_decode($zbp->Config('Neditor')->plugin)->version < 2.33) {
...
window.NEDITOR = {
    ...
    updateLog: ["升级 Neditor 上游核心源码至 v2.1.19", "上游核心：修复自动排版配置弹窗中无法设置文字两端对齐的问题", "上游核心：修复远程图片抓取插件默认配置无效的问题", "上游核心：修复部分依赖的安全问题", "修复后台配置中切换工具栏图片显示问题", "兼容新版后台样式"]
};
</script>';
